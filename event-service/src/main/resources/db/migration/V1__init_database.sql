...
(
    ...
    venue_map_id     VARCHAR(36)  NOT NULL,
...
CREATE TABLE IF NOT EXISTS eventsectionmapping
(
    id               VARCHAR(36) NOT NULL PRIMARY KEY,
    event_id         VARCHAR(36) NOT NULL,
    ...
    create_timestamp DATETIME(6),
    change_timestamp DATETIME(6),
    UNIQUE KEY uk_eventsectionmapping_event_ticket_seat_section (event_id, ticket_type_id, seat_map_id, section_id),
    KEY idx_eventsectionmapping_event_id (event_id),
    KEY idx_eventsectionmapping_ticket_type_id (ticket_type_id),
    KEY idx_eventsectionmapping_seat_map_id (seat_map_id),
    KEY idx_eventsectionmapping_section_id (section_id),
    CONSTRAINT fk_eventsectionmapping_event FOREIGN KEY (event_id) REFERENCES event (id) ON DELETE CASCADE,
    CONSTRAINT fk_eventsectionmapping_ticket_type FOREIGN KEY (ticket_type_id) REFERENCES tickettype (id) ON DELETE CASCADE,
    CONSTRAINT fk_eventsectionmapping_seat_map FOREIGN KEY (seat_map_id) REFERENCES seatmap (id) ON DELETE CASCADE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
(
    ...
    expires_at         DATETIME(6)  NOT NULL,
    status             VARCHAR(255) NOT NULL,
    stand_inventory_id VARCHAR(36)  NOT NULL,
    version            BIGINT,
    create_user        VARCHAR(255),
    change_user        VARCHAR(255),
    create_timestamp   DATETIME(6),
