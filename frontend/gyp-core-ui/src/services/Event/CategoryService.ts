...
export class CategoryService {
    static getAllCategories = async (): Promise<CategoryResponseDto[]> => {
        const res = await apiClient.get(`/${EVENT_SERVICE_PATH}/${CATEGORY_PATH}`);
        return res.data;
    }

    static getCategoryById = async (id: string): Promise<CategoryResponseDto> => {
        const res = await apiClient.get(`/${EVENT_SERVICE_PATH}/${CATEGORY_PATH}/${id}`);
        return res.data;
    }

    static createCategory = async (body: CategoryRequestDto): Promise<CategoryResponseDto> => {
        const res = await apiClient.post(`/${EVENT_SERVICE_PATH}/${CATEGORY_PATH}`, body);
        return res.data;
    }

    static updateCategory = async (body: CategoryRequestDto, id: string): Promise<CategoryResponseDto> => {
        const res = await apiClient.put(`/${EVENT_SERVICE_PATH}/${CATEGORY_PATH}/${id}`, body);
        return res.data;
    }

    static deleteCategory = async (id: string): Promise<void> => {
