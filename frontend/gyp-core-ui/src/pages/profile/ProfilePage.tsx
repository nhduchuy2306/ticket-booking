...
import { OrganizationResponseDto } from "../../models/generated/auth-service-models";
...
const ProfilePage: React.FC<ProfilePageProps> = ({collapsed = false}) => {
    ...
    const [isBoxOfficeMode, setIsBoxOfficeMode] = useState(JSON.parse(IamService.getBoxOfficeMode() || 'false'));
    const [organization, setOrganization] = useState<OrganizationResponseDto | null>();
    ...
    useEffect(() => {
        (async () => {
            try {
                const organizationId = IamService.getOrganizationId();
                if (organizationId) {
    ...
    return (
            <div className="flex flex-col items-start justify-content-center border border-gray-200 rounded-lg !m-1">
                <div className="flex items-center gap-4 bg-white  px-4! py-3! cursor-pointer">
                    {/* Profile Section */}
                ...
                <div className="flex items-center gap-2 bg-white px-4! py-3! cursor-pointer">
                    <label>Org: </label>
                    <label>{organization ? organization.name : "N/A"}</label>
                </div>
            </div>
    );
}
