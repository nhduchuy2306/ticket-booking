...
import { CustomerRegisterRequestDto } from "../../models/generated/auth-service-models";
...
const RegisterPage: React.FC = () => {
    ...
    const onFinish = async (values: CustomerRegisterRequestDto): Promise<void> => {
        setLoading(true);
        setErrorMessage(null);
        setSuccessMessage(null);
        try {
            const response = await register(values);
            setSuccessMessage(response.message || "Registration successful. You can sign in now.");
            window.setTimeout(() => {
                navigate("/login?registered=1");
            }, 2000);
        } catch (error: unknown) {
            if (axios.isAxiosError(error)) {
                setErrorMessage(error?.response?.data?.message || error?.response?.data || "Registration failed. Please try again.");
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
            <div className={authShellClassName}>
                    <AuthLayout
                            ...
                            description="Register once and use the same customer account to log in with email or Google."
                            footer={
                                <div className="mt-4 text-center text-sm leading-6 text-slate-200/80">
                                    <span>Already have an account? </span>
                                    <Link className="font-semibold text-amber-400 transition hover:text-amber-300" to="/login">
                                        Sign in
                                    </Link>
                                </div>
                            }
                    >
                        {successMessage &&
                            <Alert className="rounded-xl border border-white/10 bg-slate-900/85 text-slate-50 shadow-lg [&_.ant-alert-message]:text-slate-50 [&_.ant-alert-description]:text-slate-200" type="success" message="Registration submitted"
                                   description={successMessage} showIcon/>}
                        {errorMessage && <Alert className="rounded-xl border border-white/10 bg-slate-900/85 text-slate-50 shadow-lg [&_.ant-alert-message]:text-slate-50 [&_.ant-alert-description]:text-slate-200" type="error" message="Registration failed"
                                                description={errorMessage} showIcon/>}
                        <Form form={form} layout="vertical" className="mt-5 grid gap-4 [&_.ant-form-item]:!mb-0 [&_.ant-form-item-label>label]:font-semibold [&_.ant-form-item-label>label]:text-slate-200 [&_.ant-form-item-explain-error]:mt-1 [&_.ant-form-item-explain-error]:text-rose-400" onFinish={onFinish}
                              requiredMark={false}>
                            <Form.Item
                                    name="name"
                                    label="Name"
                            ...
                            >
                                <Input className="h-12 rounded-xl border-slate-700 bg-slate-900/85 text-slate-50 placeholder:text-slate-400 hover:border-amber-400" placeholder="Your full name" autoComplete="name"/>
                            </Form.Item>
                            ...
                            >
                                <PasswordInput className="h-12 rounded-xl border-slate-700 bg-slate-900/85 text-slate-50 placeholder:text-slate-400 hover:border-amber-400" placeholder="Create a password" autoComplete="new-password"/>
                            </Form.Item>
                            ...
                            >
                                <PasswordInput className="h-12 rounded-xl border-slate-700 bg-slate-900/85 text-slate-50 placeholder:text-slate-400 hover:border-amber-400" placeholder="Confirm your password" autoComplete="new-password"/>
                            </Form.Item>
